...
function send(options) {
    ...
    let xmlAcceptHeaders = new Set(['application/xml', 'text/xml']);
    ...
    if (options != null) {
        ...
        if (options.xmlAcceptHeaders != null && options.xmlAcceptHeaders.constructor == Array){
            xmlAcceptHeaders = new Set(options.xmlAcceptHeaders);
        } else if (options.xmlAcceptHeaders != null){
            xmlAcceptHeaders = options.xmlAcceptHeaders;
        }
        
    }

    function checkAcceptHeader(acceptHeader){
    ...
    return function (req, res, next) {
        try {
            let send = function (data) {
                if (typeof data === 'object') {
                    if (checkAcceptHeader(req.headers['accept'])) {
                        let resOptionsclear = { xmlHeader: true };
                        if (req.headers['user-agent']) {
                            resOptionsclear.escape = true;
                        }
